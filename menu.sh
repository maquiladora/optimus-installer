...
tput cup 7  3; if lsblk -o NAME -n /dev/sda2 | grep -q 'sda2'; then echo -ne "\e[32m c. Créer une partition /srv indépendante \e[0m"; else echo -ne "\e[31m c. Créer une partition /srv indépendante \e[0m"; fi
tput cup 8  3; if lsblk -o MOUNTPOINT -n /dev/sda2 | grep -q '/srv'; then echo -ne "\e[32m c. Crypter la partition /srv \e[0m"; else echo -ne "\e[31m c. Crypter la partition /srv \e[0m"; fi
