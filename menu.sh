...
tput cup 5  3; if grep -q "Port 7822" /etc/ssh/sshd_config; then echo -ne "\e[32m a. Sécuriser le serveur (facultatif mais recommandé)\e[0m"; else echo -ne "\e[31m a. Sécuriser le serveur \e[0m"; fi
tput cup 6  3; if grep -q "ChallengeResponseAuthentication yes" /etc/ssh/sshd_config; then echo -ne "\e[32m b. Sécuriser l'accès SSH avec GOOGLE AUTHENTICATOR (facultatif mais recommandé) \e[0m"; else echo -ne "\e[31m b. Sécuriser le serveur SSH avec un GOOGLE AUTHENTICATOR \e[0m"; fi
