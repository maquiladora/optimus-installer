...
rdiff-backup -v 6 --force --exclude /srv/databases --print-statistics --remote-schema "ssh -p$BACKUP_SERVER_SSHPORT -o 'StrictHostKeyChecking no' -i /root/private.pem %s sudo rdiff-backup --server" /srv autobackup@$BACKUP_SERVER::/srv >> /var/log/allspark-backup.log
