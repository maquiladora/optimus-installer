...
rdiff-backup -v 6 --force --exclude /srv/databases --print-statistics --remote-schema "ssh -i /root/private.pem %s sudo rdiff-backup --server" /srv debian@$BACKUP_SERVER::/srv >> /var/log/allspark-backup.log

ssh -i /root/private.pem -p $BACKUP_SERVER_SSHPORT debian@$BACKUP_SERVER sudo umount /backup &>> /var/log/allspark-backup.log
ssh -i /root/private.pem -p $BACKUP_SERVER_SSHPORT debian@$BACKUP_SERVER sudo rdiff-backup-fs --full /backup /srv &>> /var/log/allspark-backup.log
