...
class dossier
{
  ...
  function create($data)
  {
    ...
    $authorizations->bindParam(':email', $data->user, PDO::PARAM_INT);
    $authorizations->execute();
    $authorizations = $authorizations->fetch(PDO::FETCH_ASSOC);
    ...
    $dossier = $this->conn->prepare("INSERT INTO `" . $data->db . "`.dossiers SET nom = :nom, numero = :numero, date_ouverture = :date_ouverture");
    $dossier->bindParam(':nom', $this->nom);
    $dossier->bindParam(':numero', $this->numero);
    $dossier->bindParam(':date_ouverture', $this->date_ouverture);
    if($dossier->execute())
    {
