...
function read($db,$data)
{
  ...
  while ($setting = $settings->fetch(PDO::FETCH_ASSOC))
    $results[substr($setting['id'],strlen($data->module)+1)] = str_replace('"-0"','-0',$setting['value']);
...
function replace($db,$data)
{
  ...
  foreach($data->settings as $key => $value)
    $setting = $db->query("REPLACE INTO `" . $data->db . "`.settings VALUES('" . $data->module . "." . $key . "','" . json_encode(str_replace('"-0"','-0',$value)) . "')");
