...
class PDO extends \Sabre\DAVACL\PrincipalBackend\AbstractBackend {
    ...
    function getPrincipalsByPrefix($prefixPath) {
        ...
        $fields = [
            'CONCAT("principals/",email) as uri',
        ];

        foreach ($this->fieldMap as $key => $value) {
            $fields[] = $value['dbField'];
        }
        $result = $this->pdo->query('SELECT ' . implode(',', $fields) . '  FROM ' . $this->tableName . ' WHERE status = 1');
     ...
     */
        ...
        $stmt = $this->pdo->prepare('SELECT ' . implode(',', $fields) . '  FROM ' . $this->tableName . ' WHERE CONCAT("principals/",email) = ?');
        $stmt->execute([$path]);
    ...
    function updatePrincipal($path, DAV\PropPatch $propPatch) {
        ...
        $propPatch->handle(array_keys($this->fieldMap), function($properties) use ($path) {
            ...
            $query .= " WHERE CONCAT('principals/',email) = :uri";
            $values['uri'] = $path;
    ...
    function searchPrincipals($prefixPath, array $searchProperties, $test = 'allof') {
        ...
        $query = 'SELECT CONCAT("principals/",email) as uri FROM ' . $this->tableName . ' WHERE ';
        $values = [];
    ...
    function getGroupMemberSet($principal) {
        ...
        $stmt = $this->pdo->prepare('SELECT CONCAT("principals/",principals.email) as uri FROM ' . $this->groupMembersTableName . ' AS groupmembers LEFT JOIN ' . $this->tableName . ' AS principals ON groupmembers.member_id = principals.id WHERE groupmembers.principal_id = ?');
    ...
    function setGroupMemberSet($principal, array $members) {
        ...
        $stmt = $this->pdo->prepare('SELECT id, CONCAT("principals/",email) as uri FROM ' . $this->tableName . ' WHERE CONCAT("principals/",email) IN (? ' . str_repeat(', ? ', count($members)) . ');');
